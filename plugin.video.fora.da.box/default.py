...
addon = xbmcaddon.Addon('plugin.video.fora.da.box')
...
def makeRequest(url, headers=None):
        try:
            if headers is None:
                headers = {'User-Agent' : 'Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/52.0.2743.116 Safari/537.36 503.40'}
...
def getSources():
        try:
            if os.path.exists(favorites) == True:
                addDir('[B][COLOR deepskyblue][ Fora da Box Favoritos ][/COLOR][/B]','[B][COLOR deepskyblue][ Fora da Box Favoritos ][/COLOR][/B]',4,'http://imgur.com/4z8iGDK.png' , os.path.join(home, 'fanart.gif'),FANART,'','','','')
