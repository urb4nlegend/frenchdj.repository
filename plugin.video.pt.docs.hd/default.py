...
def GetSublinks(name,url,iconimage,fanart):
    ...
    if c==1:
        try:
         ...
         rNo=dialog.select('pt.docs.hd Select A Source', List)
         if rNo>=0:
             rName=name
             rURL=str(ListU[rNo])
...
#hakamac code
    ...
    progress.create('pt.docs.hd Searching Please wait',' ')
...
def addLink(url,name,iconimage,fanart,description,genre,date,showcontext,playlist,regexs,total,setCookie=""):
        ...
        if showcontext:
            ...
            if showcontext == 'fav':
                contextMenu.append(
                    ('[COLOR white]Remover de [B][COLOR lime]PT.DocS.HD[/COLOR][/B] [COLOR white]Favoritos[/COLOR]','XBMC.RunPlugin(%s?mode=6&name=%s)'
                     %(sys.argv[0], urllib.quote_plus(name)))
                     )
                ...
                contextMenu.append(('[COLOR white]Adicionar a [B][COLOR lime]PT.DocS.HD[/COLOR][/B] [COLOR white]Favoritos[/COLOR]','XBMC.RunPlugin(%s)' %fav_params))
            liz.addContextMenuItems(contextMenu)
...
if mode==None:
    ...
    try:
        ...
        xbmc.executebuiltin("XBMC.Notification(pt.docs.hd,Please [COLOR yellow]install the Youtube Addon[/COLOR] module ,10000,"")")
