...
def addon_log(string):
    if debug == 'true':
        xbmc.log("[addon.live.alive.hd Lists-%s]: %s" %(addon_version, string))
...
def GetSublinks(name,url,iconimage,fanart):
    ...
    if c==1:
        try:
         ...
         rNo=dialog.select('alive.hd Select A Source', List)
         if rNo>=0:
             rName=name
             rURL=str(ListU[rNo])
...
#hakamac code
    ...
    progress.create('alive.hd Searching Please wait',' ')
...
#            print 'parentalblockedpin',parentalblockedpin
            ...
            if showcontext == 'source':
                ...
                contextMenu.append(('[COLOR white] Remove from [COLOR lime][B]Alive HD[/B] [COLOR white]Favorites[/COLOR]','XBMC.RunPlugin(%s?mode=6&name=%s)'
            ...
            if not name in FAV:
                contextMenu.append(('[COLOR white]Add to  [COLOR lime][B]Alive HD[/B] [COLOR white]Favorites[/COLOR]','XBMC.RunPlugin(%s?mode=5&name=%s&url=%s&iconimage=%s&fanart=%s&fav_mode=%s)'
...
if mode==None:
    ...
    try:
        ...
        xbmc.executebuiltin("XBMC.Notification(alive.hd,Please [COLOR yellow]install the Youtube Addon[/COLOR] module ,10000,"")")
